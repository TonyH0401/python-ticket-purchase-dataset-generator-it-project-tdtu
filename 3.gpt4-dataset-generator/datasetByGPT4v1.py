...
num_records = 1000000  # Target size for the dataset
...
customer_ids = [f"CID_{i:05d}" for i in range(num_customers)]
customer_data = pd.DataFrame(
    {'CustomerName': customer_names, 'CustomerID': customer_ids})

# Generating EventIDs for the provided EventNames
event_ids = [f"EID_{i:03d}" for i in range(len(event_names))]
...
for gh in range(num_records // 10000):
    ...
    sales_data = pd.DataFrame({
        ...
        'TotalPrice': np.random.uniform(20.0, 500.0, 10000).round(2),
...
dataset.to_csv(
    'C:/Users/mtbv/Documents/1.tdtu/3.da-cntt/python-dataset/data-for-gpt4/sales_data.csv', index=False)
